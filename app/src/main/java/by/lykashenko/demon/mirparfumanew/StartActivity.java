...
//            "ORDER BY brendu.pagetitle ASC";
    ...
    private String sql_load_aromat = "SELECT distinct substring_index(value,'||',1) as value FROM modx_site_tmplvar_contentvalues WHERE tmplvarid = 69 GROUP BY value order by value ASC";

    //contentid=70 and id_load =5
    private String sql_load_nota = "SELECT distinct substring_index(value,'||',1) as value FROM modx_site_tmplvar_contentvalues WHERE tmplvarid = 70 GROUP BY value order by value ASC";

    //contentid=73 and id_load =6
    private String sql_load_country = "SELECT value " +
            "FROM `modx_site_tmplvar_contentvalues` " +
            "where tmplvarid = 73 " +
            "GROUP BY value " +
            "ORDER BY `value` ASC ";
